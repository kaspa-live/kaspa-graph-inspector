...
func main() {
	...
	kaspad.SetOnAddingBlockListener(func(block *externalapi.DomainBlock) {
		err := processing.PreprocessBlock(block)
		if err != nil {
			logErrorAndExit("Could not preprocess block before adding: %s", err)
		}
	})
